...
SEARCHABLE_FIELDS = ["full_name", "email", "phone", "reference_number"]
...
def _apply_filters(model, args):
    ...
    if search:
        ...
        clauses = [getattr(model, f).ilike(like) for f in SEARCHABLE_FIELDS if hasattr(model, f)]
    ...
    if date_from:
        try:
            query = query.filter(model.created_at >= datetime.strptime(date_from, "%Y-%m-%d"))
        except ValueError:
            pass
    if date_to:
        try:
            query = query.filter(model.created_at <= datetime.strptime(date_to, "%Y-%m-%d"))
        except ValueError:
            pass
...
def dashboard():
    ...
    for app_type, model in MODEL_MAP.items():
        for item in model.query.order_by(model.created_at.desc()).limit(5).all():
            recent.append({"type": app_type, "item": item})
    recent.sort(key=lambda r: r["item"].created_at, reverse=True)
    recent = recent[:8]

    recent_notifications = Notification.query.order_by(Notification.created_at.desc()).limit(6).all()

    return render_template(
        "admin/dashboard.html", stats=stats, recent=recent, recent_notifications=recent_notifications
    )
...
def _render_list(app_type):
    ...
    pagination = _paginated_query(model, request.args)
    return render_template(
        f"admin/{app_type}_list.html",
        pagination=pagination, items=pagination.items,
        statuses=APPLICATION_STATUSES, args=request.args,
    )
...
def _mark_notification_read_for(app_type, item_id):
    notification = Notification.query.filter_by(
        application_type=app_type, reference_id=item_id, is_read=False
    ).first()
    if notification:
...
def guidance_detail(item_id):
    ...
    if not item.is_read:
        item.is_read = True
        db.session.commit()
    _mark_notification_read_for("guidance", item_id)
    return render_template("admin/guidance_details.html", item=item, statuses=APPLICATION_STATUSES)
...
def mentor_detail(item_id):
    ...
    if not item.is_read:
        item.is_read = True
        db.session.commit()
    _mark_notification_read_for("mentor", item_id)
    return render_template("admin/mentor_details.html", item=item, statuses=APPLICATION_STATUSES)
...
def volunteer_detail(item_id):
    ...
    if not item.is_read:
        item.is_read = True
        db.session.commit()
    _mark_notification_read_for("volunteer", item_id)
    return render_template("admin/volunteer_details.html", item=item, statuses=APPLICATION_STATUSES)


# ------------------------------------------------------------ status change
@admin_bp.route("/application/<app_type>/<int:item_id>/status", methods=["POST"])
@login_required
...
# ---------------------------------------------------------------- resend
@admin_bp.route("/application/<app_type>/<int:item_id>/resend", methods=["POST"])
@login_required
def resend_email(app_type, item_id):
    ...
    if not model:
        abort(404)
    item = model.query.get_or_404(item_id)
    success = email_service.send_confirmation_email(app_type, item)
    flash("Confirmation email resent." if success else "Email could not be sent — check mail configuration.",
          "success" if success else "error")
    return redirect(request.referrer or url_for("admin.dashboard"))


@admin_bp.route("/application/<app_type>/<int:item_id>/reply", methods=["POST"])
@login_required
def reply_email(app_type, item_id):
    ...
    if not subject or not body:
        flash("Subject and message are required to send a reply.", "error")
        return redirect(request.referrer or url_for("admin.dashboard"))

    success, error = email_service.send_admin_reply(app_type, item, subject, body)
    flash("Reply sent." if success else f"Reply could not be sent: {error}", "success" if success else "error")
    return redirect(request.referrer or url_for("admin.dashboard"))
...
def mentor_resume_view(item_id):
    item = MentorApplication.query.get_or_404(item_id)
    path = resume_absolute_path(current_app.config["UPLOAD_FOLDER"], item.resume_stored_name)
    if not path:
        abort(404)
...
def mentor_resume_download(item_id):
    ...
    if not path:
        abort(404)
    return send_file(path, as_attachment=True, download_name=item.resume_original_name)


# ---------------------------------------------------------------- deletion
@admin_bp.route("/application/<app_type>/<int:item_id>/delete", methods=["POST"])
@login_required
    ...
    if app_type == "mentor":
        ...
        if path and os.path.isfile(path):
            try:
                ...
                current_app.logger.warning("Could not remove resume file for mentor #%s", item_id)

    Notification.query.filter_by(application_type=app_type, reference_id=item_id).delete()
    db.session.delete(item)
    db.session.commit()
    flash("Application deleted.", "success")
    return redirect(url_for(f"admin.{app_type}_list"))


# --------------------------------------------------------------- PDF export
@admin_bp.route("/application/<app_type>/<int:item_id>/pdf")
@login_required
def application_pdf(app_type, item_id):
    ...
    if not model:
        abort(404)
    item = model.query.get_or_404(item_id)
    buffer = build_application_pdf(app_type, item)
    return send_file(buffer, mimetype="application/pdf", as_attachment=True,
                      download_name=f"{item.reference_number}.pdf")


# ---------------------------------------------------------- list exports
@admin_bp.route("/<app_type>/export/<fmt>")
@login_required
def export_list(app_type, fmt):
    ...
    items = _apply_filters(model, request.args).limit(5000).all()

    if fmt == "csv":
        buffer = build_csv(app_type, items)
        return send_file(buffer, mimetype="text/csv", as_attachment=True, download_name=f"{app_type}_export.csv")

    buffer = build_excel(app_type, items)
    return send_file(
        buffer,
        mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        as_attachment=True, download_name=f"{app_type}_export.xlsx",
    )
...
def notifications_poll():
    ...
    latest = Notification.query.order_by(Notification.created_at.desc()).limit(5).all()
    return jsonify({
        "unread_count": unread,
        "latest": [
            {
                "id": n.id, "title": n.title, "message": n.message,
                "url": n.detail_url(), "is_read": n.is_read,
